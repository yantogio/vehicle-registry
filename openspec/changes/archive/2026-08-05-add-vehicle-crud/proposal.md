...
- **Seed data** 5 baris persis sesuai dokumen soal, disisipkan lewat kelas `DataSeeder` (`CommandLineRunner`) hanya bila tabel masih kosong; struktur tabel dibuat lewat `schema.sql`.
...
|---|---|---|
| Tabel requirement dokumen soal | Text | Dropdown, nilai `['Merah','Hitam','Biru','Abu-Abu']` |
...
3. Seed data yang diminta dokumen soal berisi Warna = Hitam/Merah/Biru (semuanya ada di daftar dropdown) dan Bahan Bakar = Bensin (tidak ada di daftar dropdown). Jika daftar itu dipasang pada Bahan Bakar, seed data yang diminta dokumen soal sendiri tidak akan bisa disimpan.
...
- **Alamat seed data**: hanya `B-7763-TXY` yang alamatnya ditentukan dokumen soal (`Jl. Achmad Yani No 89 Jakarta Pusat`); alamat empat baris lainnya diisi nilai wajar buatan sendiri.
...
- **Urutan tampilan data**: data diurutkan berdasarkan `noRegistrasi` ascending, diterapkan lewat `Sort` pada `Pageable` di sisi server. Urutan ini **berbeda dari urutan pada mockup dokumen soal**. Alasannya: dokumen soal tidak menyebutkan aturan pengurutan apa pun, sedangkan pagination membutuhkan urutan yang deterministik — tanpa `ORDER BY` eksplisit, database tidak menjamin urutan baris, sehingga baris yang sama bisa muncul di dua halaman atau terlewat sama sekali. Kolom timestamp yang bisa dipakai merekonstruksi urutan input sudah masuk Non-Goals.
...
Berikut sengaja **tidak** dikerjakan, karena tidak diminta dokumen soal dan berisiko memakan waktu pada deadline < 24 jam:
